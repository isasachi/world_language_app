...
// Simple Select component interface
...
// Using type assertions to handle complex Supabase responses
// We have to be flexible with the response structure

// Define types for application
interface Teacher {
  id: string;
  first_name: string;
  last_name: string;
...
// Interface for grades when mapped to display format
interface ExtendedGrade {
  id: string;
  student_id: string;
  skill_name: string;
  score: number;
...
// Defines the shape of grade data from the database
// This interface helps with type safety when working with grade data
interface Grade {
  id: string;
  student_id: string;
  classroom_id: string;
...
const Reports = () => {
  ...
  const { data: classrooms } = useQuery<Classroom[]>({
    ...
    queryFn: async () => {
      ...
      // Transform the response to match our Classroom interface
      return (data || []).map(item => ({
        id: item.classroom_id || "",
        name: (item.classrooms as any)?.name || "",
  ...
  const { data: students, isLoading: isLoadingStudents } = useQuery<Student[]>({
    ...
    queryFn: async () => {
      ...
      return (data || []).map(item => ({
        ...
        first_name: (item.student as any)?.first_name || "",
        last_name: (item.student as any)?.last_name || ""
      }));
    },
    ...
    const fetchReportData = async () => {
      ...
      if (reportType === "attendance") {
        ...
        const { data: attendanceData, error: attendanceError } = await supabase
          .from("attendance")
          .select("id, classroom_id, student_id, quarter_id, class_date, status, comment")
          .eq("classroom_id", selectedClassroom)
        ...
        if (gradesError) {
          ...
          for (const student of studentsWithData) {
            const studentGrade = (gradesData as Grade[] | null)?.find(grade => grade.student_id === student.id);
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const classroom = classrooms?.find(c => c.id === selectedClassroom)?.name || "";
      const teacher = teachers?.find(t => t.id === selectedTeacher)?.last_name || "";
      ...
      doc.setFontSize(14);
      doc.text(`${classroom} - ${teacher}`, doc.internal.pageSize.width / 2, 30, { align: "center" });
      ...
      if (reportType === "attendance") {
        ...
        const rows = reportData.students.map(student => {
          ...
          dates.forEach(date => {
            const record = student.attendanceRecords?.find(r => r.class_date === date);
          ...
          if (student.attendanceRecords && student.attendanceRecords.length > 0) {
            const commentRecords = student.attendanceRecords.filter(r => r.comment);
            if (commentRecords.length > 0) {
              comments = commentRecords
                .map(r => `${format(parseISO(r.class_date), "MM/dd")}: ${r.comment}`)
                .join("; ");
            }
          }
        ...
        // Define skill names based on the grade schema
        const skillMapping = [
          { key: "listening", name: "Listening" },
          { key: "reading", name: "Reading" },
          { key: "writing", name: "Writing" },
          { key: "speaking", name: "Speaking" },
          { key: "grammar_vocab", name: "Grammar & Vocabulary" },
          { key: "project", name: "Project" },
          { key: "conversation", name: "Conversation" }
        ];
        
        // Create a set of all skill names
        const allSkills = new Set<string>(skillMapping.map(skill => skill.name));
        ...
        const rows = reportData.students.map(student => {
          ...
          skills.forEach(skillName => {
            ...
            // Find the grade with matching skill name
            const grade = student.grades.find(g => g.skill_name === skillName);
            row.push(grade && typeof grade.score === 'number' ? grade.score.toString() : "-");
          });
          
          // Add comments from the grade record - all grades share the same comment
          const comments = student.grades && student.grades.length > 0 
            ? (student.grades[0].comment || "")
            : "";
      ...
      alert(`Failed to generate PDF report: ${errorMessage}. Please try again.`);
    }
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        {isLoadingStudents ? (
        ...
        ) : reportData && (selectedTeacher && selectedClassroom && selectedQuarter && selectedMonth) ? (
          <div className="overflow-x-auto">
            <div className="flex items-center gap-2 mb-4">
              <Calendar size={18} />
              <span className="font-medium">
                {format(parseISO(reportData.startDate), "MMMM d, yyyy")} - {format(parseISO(reportData.endDate), "MMMM d, yyyy")}
              </span>
            </div>
          ...
          <div className="text-center py-8 text-gray-500">
            {!selectedTeacher ? "Select a teacher to start" : 
             !selectedClassroom ? "Select a classroom" : 
             !selectedQuarter ? "Select a quarter" : 
              ...
              return (
                ...
                >
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    ...
                    status === "tardy" ? "bg-yellow-100 text-yellow-800" : ""
                  }`}>
                    {status === "present" ? "✓" :
                     status === "absent" ? "A" :
                     status === "tardy" ? "T" : "-"}
            ...
            <td className="px-6 py-4">
              <div className="text-sm text-gray-900">
                {student.attendanceRecords?.length === 0 ? (
                  <div className="text-gray-500 italic">No attendance records available</div>
                ) : student.attendanceRecords?.filter(r => r.comment).length === 0 ? (
                  <div className="text-gray-500 italic">No comments available</div>
                ) : (
                  student.attendanceRecords
                    ?.filter(r => r.comment)
                    .map((r, i) => (
...
const GradesReport = ({ reportData }: { reportData: ReportData }) => {
  // Get all unique skills from grades
  const allSkills = new Set<string>();
  
  // Add skills from grades
  reportData.students.forEach(student => {
    if (student.grades && student.grades.length > 0) {
      student.grades.forEach(grade => {
        if (grade.skill_name) {
          allSkills.add(grade.skill_name);
        }
      });
    }
  });
  
  // If no skills are found, add default skills
  if (allSkills.size === 0) {
    const defaultSkills = ["Listening", "Reading", "Writing", "Speaking", "Grammar & Vocabulary", "Project", "Conversation"];
            ...
            <td className="px-6 py-4">
              <div className="text-sm text-gray-900">
                {student.grades?.length === 0 ? (
                  <div className="text-gray-500 italic">No grade records available</div>
                ) : (
                  student.grades
                    ?.filter(g => g.comment)
                    .map((g, i) => (
                      <div key={i} className="mb-1">
                        <span className="font-medium">{g.skill_name}:</span> {g.comment}
                      </div>
                    ))
                )}
              </div>
